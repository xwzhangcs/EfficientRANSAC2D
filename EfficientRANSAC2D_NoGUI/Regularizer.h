...
public:
	...
	QString saveImage(Layer & layer, int index, int level);
	void post_processing(Layer & layer, float angle_threshold);
	void regularizerMultiRunsForLayers(const std::vector<QString> &fileNameList, const std::vector<std::pair<float, float>>& height_info, const std::vector<std::pair<std::vector<int>, std::vector<int>>>& tree_info, QString detect_file, QString config_file);
	//void saveImages();
